...
// load env vars
dotenv();
const { PRIVATE_KEY, QUICKNODE_HTTP_TRANSPORT_URL } = process.env;

// validate requirements
if (!PRIVATE_KEY) throw new Error("missing PRIVATE_KEY.");
if (!QUICKNODE_HTTP_TRANSPORT_URL)
  throw new Error("missing QUICKNODE_HTTP_TRANSPORT_URL.");
...
const client = createWalletClient({
  ...
  chain: base,
  transport: http(QUICKNODE_HTTP_TRANSPORT_URL),
...
const main = async () => {
  ...
  const quicknodeBaseUrl = `${process.env.QUICKNODE_HTTP_TRANSPORT_URL}${process.env.ZEROX_ADDON_PATH}`;
  ...
  const priceUrl = `${quicknodeBaseUrl}/swap/allowance-holder/price?${priceParams.toString()}`;
  ...
  const quoteUrl = `${quicknodeBaseUrl}/swap/allowance-holder/quote?${quoteParams.toString()}`;
